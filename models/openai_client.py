...
class OpenAIClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Get API key with explicit fallbacks and clean it
        ...
        try:
            ...
            logger.warning("OpenAI client will operate in fallback mode - content generation will use placeholders")
            self.client = None
            self._is_available = False
    ...
    def generate(
        ...
        try:
            ...
            if timeout and timeout != 30.0:  # Only create new client if timeout is different
                timeout_client = OpenAI(api_key=self.api_key, timeout=timeout)
                response = timeout_client.chat.completions.create(**kwargs)
            else:
