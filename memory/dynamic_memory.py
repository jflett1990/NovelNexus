...
class DynamicMemory:
    ...
    def _filter_memory(self, query: str, agent_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Filter memory using property:value syntax.

        Args:
            query: Filter query in format "property:value"
        ...
        property_name, value = query.split(":", 1)
        ...
        for doc_id, metadata in self.metadata.items():
            ...
            if property_name in metadata and str(metadata[property_name]) == value:
                # Apply agent filter if specified
                if agent_name is None or metadata.get('agent') == agent_name:
                    if doc_id in self.documents:
                        results.append({
                            'text': self.documents[doc_id],
                            'metadata': metadata,
                            'score': 1.0  # Perfect match for filter
                        })
    ...
    def _semantic_search(
        ...
        try:
            ...
            query_embedding = self._standardize_embedding(query_embedding)
            ...
            for doc_id, doc_embedding in self.embeddings.items():
                # Apply agent filter if specified
                if agent_name and self.metadata.get(doc_id, {}).get('agent') != agent_name:
                    continue
