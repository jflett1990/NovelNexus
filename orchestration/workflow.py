...
class ManuscriptWorkflow:
    ...
    def visualize_workflow(self) -> Dict[str, Any]:
        ...
        nodes = [
            ...
            {"id": "chapter_writing", "label": "Chapter Writing", "status": "completed" if "chapter_writing" in self.completed_stages else "pending"},
            {"id": "longform_expansion", "label": "Longform Expansion", "status": "completed" if "longform_expansion" in self.completed_stages else "pending"},
            {"id": "editorial_review", "label": "Editorial Review", "status": "completed" if "editorial_review" in self.completed_stages else "pending"},
        ...
        edges = [
            ...
            {"source": "chapter_writing", "target": "longform_expansion", "label": "Follows"},
            {"source": "longform_expansion", "target": "editorial_review", "label": "Follows"},
            {"source": "editorial_review", "target": "manuscript_assembly", "label": "Follows"}
        ]
    ...
    def _run_agent(self, agent_key: str):
        ...
        try:
            ...
            if agent_key == "ideation":
                ...
                self.agents[agent_key].create_plot(
                    integrated_data=integrated_data,
                    genre=self.genre,
                    target_length=self.target_length,
                    complexity=self.complexity
                ...
                # Mark stage as completed
                self._complete_stage("plot")

            elif agent_key == "chapter_planning":
                # Get all data so far
                integrated_data = self.central_hub.integrate_all_data()

                self.agents[agent_key].create_chapter_plan(
                    integrated_data=integrated_data,
                    target_length=self.target_length
                )
                # Mark stage as completed
                ...
                # Get all data for chapter writing
                integrated_data = self.central_hub.integrate_all_data()

                self.agents[agent_key].write_chapters(
                    integrated_data=integrated_data
                )
                # Mark stage as completed
                self._complete_stage("chapter_writing")

            elif agent_key == "expander":
                # Get chapters from chapter writer and expand them
                self._run_longform_expansion()
                # Mark stage as completed
                self._complete_stage("longform_expansion")

            elif agent_key == "editorial":
                # Run editorial review on all chapters
                self._run_editorial_review()
                # Mark stage as completed
                self._complete_stage("editorial_review")

            elif agent_key == "manuscript":
                # Assemble final manuscript
                self._run_manuscript_assembly()
                # Mark stage as completed
                self._complete_stage("manuscript_assembly")
    ...
    def execute(self, **config):
        ...
        try:
            ...
            # Stage 7: Chapter Writing (Initial Draft)
            self.current_stage = "chapter_writing"
            self._save_workflow_state()
            self._run_agent("writing")

            # Stage 8: Longform Expansion (Enrich Content)
            self.current_stage = "longform_expansion"
            self._save_workflow_state()
            self._run_agent("expander")

            # Stage 9: Editorial Review (Continuity & Quality)
            self.current_stage = "editorial_review"
            self._save_workflow_state()
            self._run_agent("editorial")

            # Stage 10: Final Manuscript Assembly
            self.current_stage = "manuscript_assembly"
            self._save_workflow_state()
    ...
    def _run_longform_expansion(self):
        """Run longform expansion on all chapters."""
        logger.info("Running longform expansion on chapters")

        # Get all chapters from chapter writer
        ...
        for doc in chapter_docs:
            try:
                ...
                if len(chapter_content) < 500:  # Only expand short chapters
                    logger.info(f"Expanding chapter {chapter_data.get('number', '?')}")

                    # Use progressive expansion for richer content
                    ...
                    # Update chapter with expanded content
                    ...
                    # Chapter is already long enough, just enhance dialogue
                    enhanced_content = self.agents["expander"].enhance_dialogue(
                        text=chapter_content,
                        style="literary",
                        themes=chapter_data.get("themes", [])
                    )

                    chapter_data["content"] = enhanced_content
                    chapter_data["word_count"] = len(enhanced_content.split())
                    chapter_data["enhanced"] = True

                    expanded_chapters.append(chapter_data)

            except Exception as e:
                logger.error(f"Error expanding chapter: {str(e)}")
                continue
        ...
        logger.info(f"Completed longform expansion on {len(expanded_chapters)} chapters")

    def _run_editorial_review(self):
        """Run editorial review on all expanded chapters."""
        logger.info("Running editorial review on chapters")

        # Get expanded chapters
        expanded_docs = self.memory.query_memory("type:expanded_chapter", agent_name="longform_expander")

        if not expanded_docs:
            # Fallback to regular chapters if no expanded ones
        ...
        # Get integrated data for style guide
        integrated_data = self.central_hub.integrate_all_data()
        style_guide = {
            "genre": integrated_data.get("genre", ""),
            "themes": integrated_data.get("themes", []),
            "tone": "professional",
            "target_audience": "general"
        }
        ...
        for doc in expanded_docs:
            try:
                ...
                logger.info(f"Editorial review of chapter {chapter_data.get('number', '?')}")

                # Run editorial review
                reviewed_chapter = self.agents["editorial"].edit_chapter(
        ...
        logger.info(f"Completed editorial review on {len(reviewed_chapters)} chapters")

    def _run_manuscript_assembly(self):
        """Assemble the final manuscript from all reviewed chapters."""
        logger.info("Assembling final manuscript")

        # Get all reviewed chapters
        reviewed_docs = self.memory.query_memory("type:edited_chapter", agent_name="editorial_agent")

        if not reviewed_docs:
            # Fallback to expanded chapters
            reviewed_docs = self.memory.query_memory("type:expanded_chapter", agent_name="longform_expander")

        if not reviewed_docs:
            # Final fallback to original chapters
            reviewed_docs = self.memory.query_memory("type:chapter", agent_name="chapter_writer_agent")

        if not reviewed_docs:
            logger.error("No chapters found for manuscript assembly")
            return

        # Convert documents to chapter data
        chapters = []
        for doc in reviewed_docs:
            try:
                ...
                logger.error(f"Error parsing chapter data: {str(e)}")
                continue

        # Get integrated data for book info
        integrated_data = self.central_hub.integrate_all_data()
        book_info = {
            "title": integrated_data.get("title", "Untitled Novel"),
            "author": "AI Author",
            "genre": integrated_data.get("genre", "Fiction")
        }

        # Assemble the manuscript
        final_manuscript = self.agents["manuscript"].assemble_manuscript(chapters)

        logger.info(f"Final manuscript assembled with {len(chapters)} chapters and {final_manuscript.get('word_count', 0)} words")
